...
use talpid_types::{
    net::{
        openvpn, wireguard, Endpoint, TransportProtocol, TunnelEndpoint, TunnelParameters,
        TunnelType,
    },
    ...
    L: EventListener + Clone + Send + 'static,
{
    pub async fn start(
        ...
        let initial_target_state = if account_manager.is_some() {
            ...
            let result = match self.settings.get_relay_settings() {
                ...
                RelaySettings::Normal(constraints) => {
                    ...
                    if let Some((relay, endpoint)) = endpoint {
                        let result = self
                            .create_tunnel_parameters(&relay, endpoint, device.token, retry_attempt)
                            .await;
    ...
    async fn set_account(&mut self, account_token: Option<String>) -> Result<bool, Error> {
        ...
        let result = match previous_token {
            Some(token) => self.account_history.set(token).await,
            None => self.account_history.clear().await,
        };
